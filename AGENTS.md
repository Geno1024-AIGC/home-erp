...
- Channel mapping: in-app update channel **Canary** = GitHub pre-releases; **正式版** (stable) = regular GitHub releases (none published yet). The api metadata is always read from `api.github.com`; the APK file is downloaded through the user-selected **update source** (GitHub or mirror prefix). See `satellite/android/.../update/Updater.kt`.
- APK signing: both debug and release variants sign with the repo-tracked standard debug keystore `satellite/android/signing/debug.jks` (`androiddebugkey` / `android`), giving a stable signature across CI runs so a Canary update can install over the previous one. Treat it as the throwaway canary key; a future stable release key should come from a secret (e.g. `KEYSTORE_BASE64`) instead of the repo.
...
Commits must be **atomic** — one logical change per commit. Dense, very frequent commits are encouraged and expected (small steps beat big batches). Split a feature into many small commits — one commit per logical sub-fix/sub-step — instead of bundling them together; `./gradlew build`/pack-counter churn normally rides along the commit that caused the build.
